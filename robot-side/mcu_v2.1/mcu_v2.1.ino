...
// Function prototypes
int commandRoutine();
int commandRoutineResource();
int sensorRoutine(); 

// Threads!
#define T_STATUS_WAITING    0
#define T_STATUS_BLOCKED    1
#define T_STATUS_ACTIVE     2

#define T_PRIORITY_HIGH     2
...
struct PriorityThread controller = {
  0,
  &commandRoutine,
  T_PRIORITY_HIGH,
  T_STATUS_WAITING,
  &commandRoutineResource
};

struct PriorityThread sensor = {
  0,
  &sensorRoutine,
  T_PRIORITY_LOW,
  T_STATUS_WAITING,
...
void runThread(struct PriorityThread *thread) {
  thread->threadStatus = T_STATUS_ACTIVE;

  if (thread->routine() < 0) {
    thread->threadStatus = T_STATUS_BLOCKED;
  }
  else {
    thread->threadStatus = T_STATUS_WAITING;
  }
}
...
int commandRoutine () {
  static int nextByteType = START_TYPE; // What is the next byte to expect?
  static int motorIdentifier = 0; // What motor to expect a value for?
  static int sensorIdentifier = 0; // What motor to expect a value for?
